...
export function Collage({ queryData }) {
  ...
  return (
    ...
    <div className="relative grid grid-rows-3 lg:grid-rows-1 lg:grid-cols-3 min-h-[382px] xs:min-h-[448px] sm:min-h-[488px] lg:min-h-[800px] max-h-screen max-w-[1000px] xl:max-w-[1400px]">
      <div className="absolute w-full flex flex-wrap justify-center items-center gap-2 p-2 z-10 my-auto">
        {images.length > 0 && images.map((img, index) => (
          <img
            ...
            alt={img.alt}
            height={img.height}
            width={img.width + img.width * (1/3)}
            class:list={[index > 0 ? 'hidden' : "", `object-cover aspect-auto shadow-md my-0 ${index < 2
              ? "block"
              : index < 3
                ? "hidden md:block"
                : index < 5
