...
import type { UwContactInformationFields } from "@/__generated__/types";
...
export async function getFooterData(): Promise<FooterComponentQueryResponse> {
  try {
    ...
    return getDefaultProps<FooterComponentQueryResponse>(
...
export async function createProps(): Promise<Props> {
  try {
    ...
    const informationLinks = informationLinkConfig.map(
      (linkObj: InformationLink) => {
        const field = wpContent[linkObj.queryField as keyof typeof wpContent];
        if (!field?.nodes?.[0]) return { ...linkObj, url: undefined };

        return {
          url: (field.nodes[0] as any)[linkObj.querySubField]?.pdfFile?.node
    ...
    const contactInfo =
      wpContent?.uwContactInfos?.nodes[0].uWContactInformationFields;
    ...
    const socials = content.socials.map((social: SocialLink) => {
      return {
        ...social,
        link: contactInfo[
          social.queryField as keyof UwContactInformationFields
        ] as string,
      };
    });
