...
<Sheet>
  <SheetTrigger class="absolute lg:hidden right-4 top-3">
    <Button
      variant={"outline"}
      class="text-2xl border-0 shadow-none p-0 h-min"
    ...
    >
      <img
        loading="lazy"
        decoding="async"
        class="w-[24px] text-primary-blue-2 font-semibold"
  ...
  >
    <nav aria-label="Main navigation" >
      ...
      <div
        class="mt-2 ml-4 w-full flex flex-col items-start space-y-2 text-white text-xl text-left"
        role="menu"
      >
        {#if showGoBackBtn }
          <div
            class="border-b-2 border-primary-blue-1 w-full py-2"
            role="none"
          >
      ...
      <!-- DONATE BUTTON -->
      <div
        class="flex flex-col items-center justify-center w-full list-none mt-4"
        role="none"
      >
        <LearnMoreBtn
          text="Donate"
