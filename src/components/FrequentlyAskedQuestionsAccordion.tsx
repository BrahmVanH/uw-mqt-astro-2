...
import type { RootQueryToFaqConnection } from "@/__generated__/types";
...
export function createProps(
  graphQLQueryResponse: RootQueryToFaqConnection,
): Props {
...
const FrequentlyAskedQuestionsAccordion: React.FC<Props> = ({
  title,
  items,
}) => {
      ...
      >
        {items?.map((item, index) => (
          <AccordionItem
            key={index}
            value={`item-${index}`}
            className="group  px-4 border-none  transition-all duration-300lg:hover:shadow-[0_0_15px_rgba(0,68,181,0.05)]"
          >
            ...
            <AccordionContent className="animate-fade-in overflow-hidden text-md">
              <div>
                <p className="text-tertiary-black-2 leading-relaxed">
                  {item.answer}
                </p>
              </div>
            </AccordionContent>
