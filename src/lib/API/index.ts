...
import {
  ...
  WP_API_USERNAME,
  WP_API_PASSWORD,
  YOOPERS_UNITED_API_TOKEN,
  YOOPERS_UNITED_API_ROOT_URL,
...
export async function getCachedData<T>(
  ...
  try {
    if (import.meta.env.PROD) {
      const cachedData = await retryOperation(
        () => redis.get(key),
        retryAttempts,
      );
  ...
  try {
    const data = await getCachedData(cacheKey, async () => {
      const response = await fetch(
        `${YOOPERS_UNITED_API_ROOT_URL}/needs?per_page=100&page=7&need_status=active&show_inactive=No&sort=desc&sort_by=date`,
        {
          method: "GET",
          headers: {
      ...
      const validNeeds = responseData.data
        .filter((need: any) => {
          ...
          return (
            !isNaN(needDate.getTime()) &&
            need.agency.agency_name !== "Sample Agency" &&
            (need.need_date_type === "until" ||
              need.need_date_type === "multi" ||
              needDate >= today)
          );
        })
        .slice(0, 10);
    ...
    return getDefaultProps<YoopersUnitedNeedsFetchResponse>(
      "VolunteerNeeds fetchGalaxyDigitalNeedsData",
    );
  }
}
